...
def jobman(state, channel):
    ...
    x = TT.lmatrix('x')
    x_mask = TT.lmatrix('x_mask')
    y = TT.lmatrix('y')
    y_mask = TT.lmatrix('y_mask')
    ...
    rec_layer = rec(x_emb, x_mask, n_steps=x.shape[0],
                    init_state=h0*reset,
    ...
    #### Training
    if state['shortcut_inpout']:
        # note the output layer doesn't take the x_mask. 
        train_model = output_layer(rec_layer,
                                   no_noise_bias=state['no_noise_bias'],
                                   additional_inputs=[shortcut(x)]).train(target=y,
                mask=y_mask, scale=numpy.float32(1./state['seqlen']))
    else:
        train_model = output_layer(rec_layer, x_mask
